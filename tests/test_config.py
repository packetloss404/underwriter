...
from underwriter.config import (
    LIVE_TRADING_HOST,
    PAPER_TRADING_HOST,
    LiveTradingBlocked,
    RiskLimits,
...
LEAKY_VARS = [
    *CREDS,
    ...
    "ANTHROPIC_API_KEY",
    "UNDERWRITER_ENV",
    "UNDERWRITER_KILL_SWITCH",
    "UNDERWRITER_RISK_MAX_RISK_PER_TRADE_PCT",
    "UNDERWRITER_RISK_MAX_CONCURRENT_POSITIONS",
    "UNDERWRITER_RISK_MAX_TOTAL_OPEN_RISK_PCT",
    "UNDERWRITER_RISK_DAILY_LOSS_STOP_PCT",
    "UNDERWRITER_RISK_MAX_QUOTE_AGE_SECONDS",
    "UNDERWRITER_RISK_MIN_DAYS_TO_EXPIRY",
    "UNDERWRITER_RISK_MAX_DAYS_TO_EXPIRY",
]
...
class TestFailClosed:
    ...
    def test_kill_switch_defaults_off_but_is_settable(
        ...
        _env(monkeypatch, UNDERWRITER_KILL_SWITCH="true")
    ...
    def test_unknown_environment_name_is_rejected(self, monkeypatch: pytest.MonkeyPatch) -> None:
        _env(monkeypatch, UNDERWRITER_ENV="production")
        with pytest.raises(ValidationError):
            Settings()
...
class TestRiskLimits:
    ...
    def test_position_cap_contradicting_aggregate_cap_is_rejected(
        ...
        monkeypatch.setenv("UNDERWRITER_RISK_MAX_RISK_PER_TRADE_PCT", "1.0")
        monkeypatch.setenv("UNDERWRITER_RISK_MAX_CONCURRENT_POSITIONS", "5")
    ...
    def test_inverted_expiry_window_is_rejected(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("UNDERWRITER_RISK_MIN_DAYS_TO_EXPIRY", "14")
        monkeypatch.setenv("UNDERWRITER_RISK_MAX_DAYS_TO_EXPIRY", "5")
    ...
    @pytest.mark.parametrize(
        ("var", "value"),
        [
            ("UNDERWRITER_RISK_MAX_RISK_PER_TRADE_PCT", "0"),
            ("UNDERWRITER_RISK_MAX_RISK_PER_TRADE_PCT", "-1"),
            ("UNDERWRITER_RISK_MAX_CONCURRENT_POSITIONS", "0"),
            ("UNDERWRITER_RISK_DAILY_LOSS_STOP_PCT", "0"),
            ("UNDERWRITER_RISK_MAX_QUOTE_AGE_SECONDS", "0"),
        ],
    )
    ...
    def test_typo_in_a_risk_limit_is_rejected_rather_than_ignored(
        ...
        monkeypatch.setenv("UNDERWRITER_RISK_MAX_RISK_PER_TRADE_PCNT", "0.1")
